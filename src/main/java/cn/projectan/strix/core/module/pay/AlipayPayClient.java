...
public class AlipayPayClient extends StrixPayClient {
    ...
    public Map<String, String> createWapPay(PayOrder payOrder) {
        ...
        model.setBody(payOrder.getPaymentTitle());
        model.setSubject(payOrder.getPaymentTitle());
        model.setTotalAmount(Arithmetic.div(payOrder.getTotalAmount(), 100, 2) + "");
        model.setPassbackParams(payOrder.getPaymentAttach());
...
//            model.setExtendParams(extendParams);
        try {
            HttpServletResponse response = ServletUtils.getResponse();
            AliPayApi.tradePage(response, model, config.getReturnUrl(), config.getNotifyUrl());
            return new HashMap<>();
        } catch (Exception e) {
            log.error("Strix Pay: 支付宝支付下单失败. (支付宝支付下单异常)", e);
        }

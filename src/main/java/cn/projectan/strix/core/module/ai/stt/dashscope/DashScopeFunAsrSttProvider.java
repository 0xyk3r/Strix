...
/**
 * 阿里云百炼 fun-asr / fun-asr-mtl 等离线录音文件识别 Provider。
 * <p>异步 run-task 协议。支持说话人分离、语种、热词；不支持情感与顺滑/标点/ITN。
 *
 * @author ProjectAn
 * @since 2026-06-19
 */
@Component
public class DashScopeFunAsrSttProvider extends AbstractDashScopeAsyncSttProvider {

    public DashScopeFunAsrSttProvider(DashScopeHttpClient dashScopeHttpClient) {
    ...
    public boolean supports(AiModelConfig config) {
        ...
        return (model.contains("fun-asr") || model.contains("funasr")) && !model.contains("realtime");
    }

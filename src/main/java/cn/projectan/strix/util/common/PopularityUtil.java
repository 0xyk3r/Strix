...
public class PopularityUtil {
    ...
    private long getOriginalValue(String configKey, String dataId) {
        Long value = redisUtil.zGet(StrixRedisKeyConst.HASH_POPULARITY_DATA_PREFIX + configKey, dataId);
    ...
    public void incr(String configKey, String dataId) {
        redisUtil.zIncr(StrixRedisKeyConst.HASH_POPULARITY_DATA_PREFIX + configKey, dataId);
    }
    ...
    public void syncToDB() {
        ...
        Set<String> redisConfigKeys = redisUtil.scan(StrixRedisKeyConst.HASH_POPULARITY_DATA_PREFIX + "*");
        List<String> configKeyList = redisConfigKeys.stream()
                .map(key -> key.replace(StrixRedisKeyConst.HASH_POPULARITY_DATA_PREFIX, ""))
                .toList();
        // 从数据库加载配置列表
    ...
    public void syncFormDB(boolean force) {
        ...
        for (String key : configKeyList) {
            ...
            redisUtil.zSet(StrixRedisKeyConst.HASH_POPULARITY_DATA_PREFIX + key, tuples);
        }
    }

...
public class DictSyncInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            // 2. 一次性查询所有数据库数据
            Map<String, Dict> dbDictMap = loadDbDicts();
     ...
     * 一次性加载数据库中的所有字典
     */
    private Map<String, Dict> loadDbDicts() {
    ...
    private static class BatchOperations {
        List<Dict> dictsToInsert = new ArrayList<>();
        List<Dict> dictsToUpdate = new ArrayList<>();
    ...
    private void collectBatchOperations(Dict dict, List<DictData> dictDataList,
        ...
        if (dbDict == null) {
            // 新字典，需要插入
            batchOps.dictsToInsert.add(dict);
            ...
            if (!dbDict.getName().equals(dict.getName()) || !dbDict.getDataType().equals(dict.getDataType())) {
                ...
                batchOps.dictsToUpdate.add(dbDict);
                batchOps.dictKeysNeedVersionUpdate.add(key);
            }
    ...
    private void executeBatchOperations(BatchOperations batchOps) {
        // 1. 批量插入字典
        if (!batchOps.dictsToInsert.isEmpty()) {
            dictService.saveBatch(batchOps.dictsToInsert);
            log.info("Strix Dict: 插入字典 {} 条", batchOps.dictsToInsert.size());
        }

        // 2. 批量更新字典（直接使用 MyBatis-Plus updateById，不走 DictService）
        if (!batchOps.dictsToUpdate.isEmpty()) {
            dictService.updateBatchById(batchOps.dictsToUpdate);
            log.info("Strix Dict: 更新字典 {} 条", batchOps.dictsToUpdate.size());
        }
    ...
    private void updateDictVersions(Set<String> dictKeys) {
        List<Dict> dictsToUpdate = dictService.lambdaQuery()
                .in(Dict::getKey, dictKeys)
                .list();

        dictsToUpdate.forEach(dict -> dict.setVersion(dict.getVersion() + 1));
        dictService.updateBatchById(dictsToUpdate);
    }

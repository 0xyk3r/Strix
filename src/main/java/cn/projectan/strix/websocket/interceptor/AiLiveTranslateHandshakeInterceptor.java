...
/**
 * AI 实时语音翻译（LiveTranslate）WebSocket 握手拦截器。
 * <p>
 * 验证用户 Token 并提取 configKey 查询参数，存入 WebSocket Session 属性中，
 * 供 {@link cn.projectan.strix.websocket.handler.AiLiveTranslateWebSocketHandler} 使用。
 * </p>
 *
 * <p>连接示例：{@code ws://host/ws/ai/live-translate?token=<token>&configKey=<key>}</p>
 *
 * @author ProjectAn
 * @since 2026-06-30
 */
...
public class AiLiveTranslateHandshakeInterceptor implements HandshakeInterceptor {
    ...
    public boolean beforeHandshake(@NonNull ServerHttpRequest request,
        ...
        if (!StringUtils.hasText(token)) {
            log.warn("LiveTranslate WebSocket 握手失败: Token 为空");
            return false;
        }
        ...
        if (manager == null || manager.getSystemManager() == null) {
            log.warn("LiveTranslate WebSocket 握手失败: Token 无效或已过期");
            return false;
        }

        String configKey = getQueryParam(request, "configKey");
        if (!StringUtils.hasText(configKey)) {
            log.warn("LiveTranslate WebSocket 握手失败: configKey 为空");
            return false;
        }

        attributes.put("userId", manager.getSystemManager().getId());
        attributes.put("configKey", configKey);
        log.info("LiveTranslate WebSocket 握手成功: managerId={}, configKey={}",
                manager.getSystemManager().getId(), configKey);
        return true;
    }

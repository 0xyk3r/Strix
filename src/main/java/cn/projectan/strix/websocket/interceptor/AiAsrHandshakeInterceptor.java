...
/**
 * AI 实时 ASR WebSocket 握手拦截器
 * <p>
 * 验证用户 Token 并提取 configKey 查询参数，存入 WebSocket Session 属性中，
 * 供 {@link cn.projectan.strix.websocket.handler.AiAsrWebSocketHandler} 使用。
 * </p>
 *
 * <p>连接示例：{@code ws://host/ws/ai/asr?token=<token>&configKey=<key>}</p>
 *
 * @author ProjectAn
 * @since 2026-05-21
 */
...
public class AiAsrHandshakeInterceptor implements HandshakeInterceptor {
    ...
    public boolean beforeHandshake(@NonNull ServerHttpRequest request,
        ...
        // 获取 Token（query param 或 Authorization header）
        ...
        if (!StringUtils.hasText(token)) {
            log.warn("ASR WebSocket 握手失败: Token 为空");
            return false;
        }
        ...
        if (!StringUtils.hasText(configKey)) {
            log.warn("ASR WebSocket 握手失败: configKey 为空");
            return false;
        }
